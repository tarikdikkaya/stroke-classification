...
# ------------------------- #
# 📌 Swin-L + ML-Decoder Modeli
# ------------------------- #
class SwinMLDecoder(nn.Module):
    def __init__(self, num_classes=2):
        super(SwinMLDecoder, self).__init__()
        self.backbone = timm.create_model("swin_large_patch4_window12_384", pretrained=True, num_classes=0)
        self.decoder = timm.create_model("ml_decoder", pretrained=True, num_classes=num_classes)
        self.classifier = nn.Linear(self.decoder.num_features, num_classes)
...
model = SwinMLDecoder(num_classes=2).to(device)
