...
transform = transforms.Compose([
    transforms.Resize((480, 480)),  # EfficientNetV2-L için önerilen giriş boyutu
...
# ------------------------- #
# 📌 EfficientNetV2-L Modeli
# ------------------------- #
class EfficientNetV2LModel(nn.Module):
    def __init__(self, num_classes=2):
        super(EfficientNetV2LModel, self).__init__()
        self.backbone = timm.create_model("tf_efficientnetv2_l", pretrained=True, num_classes=num_classes)
    
    def forward(self, x):
        return self.backbone(x)

model = EfficientNetV2LModel(num_classes=2).to(device)
...
best_acc = 0.0
best_model_path = "best_model_EfNetV2_L.pth"
