...
transform = transforms.Compose([
    transforms.Resize((384, 384)),  # CvT-W24 için önerilen giriş boyutu
...
# ------------------------- #
# 📌 CvT-W24 (Convolutional Vision Transformer) Modeli
# ------------------------- #
class CvTModel(nn.Module):
    def __init__(self, num_classes=2):
        super(CvTModel, self).__init__()
        self.backbone = timm.create_model("cvt_w24", pretrained=True, num_classes=num_classes)
    
    def forward(self, x):
        return self.backbone(x)

model = CvTModel(num_classes=2).to(device)
...
best_acc = 0.0
best_model_path = "best_model_cvt_w24.pth"
